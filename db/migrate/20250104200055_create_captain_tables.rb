class CreateCaptainTables < ActiveRecord::Migration[7.0]
  def up
    # A saltar esta migração inteira porque a extensão 'vector' não está disponível no EasyPanel.
    # Isto irá impedir a criação das funcionalidades de IA (Captain), mas a aplicação principal funcionará.
    puts "A saltar a migração CreateCaptainTables devido à extensão 'vector' indisponível."
  end

  def down
    # Como a migração 'up' foi saltada, a migração 'down' não precisa de apagar nenhuma tabela.
    puts "A saltar a migração 'down' para CreateCaptainTables."
  end

  # O código original foi mantido abaixo, comentado, apenas para referência futura.
  # private
  #
